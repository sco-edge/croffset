...
static const char *__doc__ =
	"croffset - compensating in-host RTT offsets";
...
#include "croffset.h" //common structs for user-space and BPF parts
#include "lhist.h"
...
static const struct option long_options[] = {
	...
	{ "vxlan",                no_argument,       NULL, 'V' }, // Calculate and report RTTs for VXLAN traffic (with encapuslated TCP timestamps)
...
static int parse_arguments(int argc, char *argv[], struct pping_config *config)
{
	...
	config->bpf_config.track_vxlan = false;
	...
	while ((opt = getopt_long(argc, argv, "hflTCVsi:r:R:t:c:F:I:x:a:4:6:w:",
				  long_options, NULL)) != -1) {
		switch (opt) {
		case 'i':
			len = strlen(optarg);
		...
		case 'C':
			...
			break;
		case 'V':
			config->bpf_config.track_vxlan = true;
			break;
		case 's':
...
const char *tracked_protocols_to_str(struct pping_config *config)
{
	...
	bool vxlan = config->bpf_config.track_vxlan;
...
static int init_rodata(struct bpf_object *obj, void *src, size_t size)
{
	...
	bpf_object__for_each_map(map, obj) {
		if (strstr(bpf_map__name(map), ".rodata")) {
			fprintf(stderr, "size: %ld, obj_size: %d\n", size, bpf_map__value_size(map));
...
static void print_event_standard(FILE *stream, const union pping_event *e)
{
	...
	} else if (e->event_type == EVENT_TYPE_TS) {
		print_ns_raw(stream, e->ts_event.timestamp);
		fprintf(stream, " 0x%x\n", e->ts_event.mark);
	}
}
